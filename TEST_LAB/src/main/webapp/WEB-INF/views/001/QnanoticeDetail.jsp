...
%>
	...
	function modifyNotice (no){
		location.href = "/QnaNotice/QnaModifyForm?nNo="+no+"&pageNo="+'${paramVO.pageNo}'+"&keyword="+encodeURI(encodeURIComponent('${paramVO.keyword}'))+'&searchName='+encodeURI(encodeURIComponent('${paramVO.searchName}'))+"&startDate="+'${paramVO.startDate}'+"&endDate="+'${paramVO.endDate}'+"&tbType=qna";
	}
	...
	function deleteNotice(no){
		if(confirm("삭제하시겠습니까? ")){
			var URL = "/QnaNotice/QnaNoticeDelete";
			$.ajax({
				type:"POST",
				url:URL,
				data:{"nNo": no,
	...
	function fileDownload(fmNo, tbkey){
		location.href="/file/fileDownload?fmNo="+fmNo+"&tbkey="+tbkey+"&tbType=qna";
	}
	...
	function registReply(nNo){
		...
		if(confirm("댓글 등록 하시겠습니까? ")){
			var URL = "/QnaNotice/replyRegistByNo";
			$.ajax({
				type:"POST",
				url:URL,
				data:{"tbKey": nNo,
				...
				success:function(data) {
					if(data.status == 'success'){
						...
						location.href = "/QnaNotice/getQnaNoticeDetail?nNo="+'${paramVO.nNo}'+"&pageNo="+'${paramVO.pageNo}'+"&keyword="+encodeURI(encodeURIComponent('${paramVO.keyword}'))+'&searchName='+encodeURI(encodeURIComponent('${paramVO.searchName}'))+"&startDate="+'${paramVO.startDate}'+"&endDate="+'${paramVO.endDate}'+"&tbType="+'${paramVO.tbType}';
					} else if( data.status == 'fail' ){
						alert(data.msg);
			        } else {
			        	alert("오류가 발생하였습니다.");
	...
	function deleteReply(nNo,cNo){
		if(confirm("댓글삭제하시겠습니까? ")){
			var URL = "/QnaNotice/replyDeleteByNo";
			$.ajax({
				type:"POST",
				url:URL,
				data:{"tbKey": nNo,
	...
	function modifyReply(tbKey,cNo,obj){
		...
		var commentTemp = $(obj).parent().prev().children().eq(0).val();
	 	...
	 	if(confirm("해당댓글수정하시겠습니까? ")){
			var URL = "/QnaNotice/ReplyUpdateByNo";
			$.ajax({
				type:"POST",
				url:URL,
				data:{"tbKey": tbKey,
...
<%-- 	<span class="path">
	...
	<section class="type01">
		...
		<div class="group01" >
			...
			<span class="font04">제목 : ${QnanoticeView.title }</span><br>
			<span class="font18">작성자 :  ${QnanoticeView.userName }<strong>&nbsp;|&nbsp;</strong> 작성일 :  ${fn:substring(QnanoticeView.regDate,0,19)}</span>
			</div>
			<div class="main_tbl">
				<ul class="notice_view">
					<li>
						...
						<div class="text_view">${QnanoticeView.content}</div> 
						<!-- 첨부파일 있을시 노출-->
						<ul class="view_list_file">
							<c:forEach items="${QnafileView}" var="item" varStatus="status">
								<li>
									<span>
										<a  href="/QnaNotice/fileDownload2?fmNo=${item.fmNo }&tbkey=${item.tbkey}&tbType=qna">${item.subSequence}</a>
									</span>
								</li>
							</c:forEach>
						</ul>
			...
			<textarea rows="" cols="" id="comment"></textarea>
			<input type="button" value="등록" onclick="registReply('${QnanoticeView.nNo}')" class="btn_admin_red"> 
		
		<c:if test="${fn:length(replyList) ne 0 }">
			<c:forEach items="${replyList}" var="item" varStatus="status">
				<ul>
					<li>작성자:${item.regUserId}</li>
					<li>내용:${item.comment}</li>
					<li>작성일:${item.regDate}</li>
				</ul>
				<c:if test="${sessionId eq item.regUserId}">
					<input type="button" value="삭제"  class="btn_admin_gray" onclick="deleteReply('${QnanoticeView.nNo}','${item.cNo}'); return false;">
				</c:if>
			</c:forEach>
		</c:if>
			...
			<div class="btn_box_con" >
				...
				<input type="button" value="수정" class="btn_admin_red" onclick="modifyNotice('${QnanoticeView.nNo}');  return false;"> 
					<input type="button" value="삭제"  class="btn_admin_gray" onclick="deleteNotice('${QnanoticeView.nNo}'); return false;">
			</div>		
		</div>
	</section> --%>
	<div class="wrap_in" id="fixNextTag">
		...
		<div class="notice_title">
			<span class="font17">${QnanoticeView.title}</span><br/>
			<span class="font18">작성자 :  ${QnanoticeView.userName} <strong>&nbsp;|&nbsp;</strong> <img src="../resources/images/btn_calendar2.png" style=" margin-top:-2px;"> 작성일 :  ${fn:substring(QnanoticeView.regDate,0,19)}</span>
		</div>
	<div class="main_tbl">
			<ul class="notice_view">
				<li>
					<div class="text_view">${strUtil:getHtml(fn:replace(QnanoticeView.content,br,crcn))}</div>
				<!-- 첨부파일 start 없을때는 아예 노출 하지 않도록 -->
					<c:if test="${fn:length(QnafileView) !=0 }">
							...
							<ul>
								<li>
									<dt>첨부파일</dt><dd>
										<c:forEach items="${QnafileView}" var="item" varStatus="status">
										<ul>
											<li><a  href="/file/fileDownload?fmNo=${item.fmNo }&tbkey=${item.tbkey}&tbType=qna">${item.orgFileName}</a> (${fn:substring(QnanoticeView.regDate,0,19)})</li>
										</ul>
										</c:forEach>
									</dd>
								</li>
								...
								<!-- 댓글영역 start-->
								<div class="comment_txt">댓글(<strong>${fn:length(replyList)}</strong>) <i>|</i> 조회수(<strong>${QnanoticeView.hits}</strong>)</div>
								<div class="comment_box">
								<div class="insert_comment">
									<table style=" width:100%">
										<tr><td><textarea style="width:100%; height:70px" placeholder="댓글을 입력하세요" id="comment"></textarea></td><td width="130px"><button style="width:100%" onclick="registReply('${QnanoticeView.nNo}'); return false;">등록</button></td></tr>
									</table>
								</div>
							...
							<c:if test="${fn:length(replyList) ne 0 }">
								...
								<c:forEach items="${replyList}" var="item" varStatus="status">
									<div class="comment_obj">
										...
										<c:if test="${sessionId eq item.regUserId}">
											<ul class="list_ul">
												...
												<li><button class="btn_doc"  onclick="deleteReply('${QnanoticeView.nNo}','${item.cNo}'); return false;"><img src="../resources/images/icon_doc04.png"> 삭제</button></li>
											</ul>  
										</c:if>
									</div>
									<div class="comment_obj" style="display:none">
										<div class="insert_comment">
											<table style=" width:100%">
												<tr><td><textarea style="width:100%; height:70px" placeholder="댓글을 입력하세요" >${fn:replace(item.comment,br,crcn)}</textarea></td>
													<td width="130px"><button style="width:100%" onclick="modifyReply('${QnanoticeView.nNo}','${item.cNo}',this); return false;">등록</button></td></tr>
											</table>
										</div>
										<ul class="list_ul">
...
<!-- 									<div class="comment_obj rec ">
			...
			<div class="btn_box_con4"><button class="btn_admin_navi" onclick="modifyNotice('${QnanoticeView.nNo}');  return false;">게시글 수정</button> <button class="btn_admin_gray" onclick="deleteNotice('${QnanoticeView.nNo}'); return false;">게시글 삭제</button> </div>
