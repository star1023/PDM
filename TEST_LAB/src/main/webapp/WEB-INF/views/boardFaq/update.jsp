...
$(document).ready(function () {
	CreateEditor('answer');
	loadCode('FAQ_CATEGORY', 'faqCategory', '${faqData.data.CATEGORY}');
	$('#title').val('${faqData.data.QUESTION}');
});

function loadCode(codeId, selectBoxId, selectedValue) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(list, function(index, value) {
			    var isSelected = selectedValue == value.itemCode;
			    $("#" + selectBoxId).addOption(value.itemCode, value.itemName, isSelected);

			    if (isSelected) {
			        $("#" + selectBoxId + "_label").text(value.itemName);
			    }
			});
		},
		error: function() {
...
function fn_update() {
    ...
    var answer = window.editor.getData();
    ...
    formData.append("idx", "${faqData.data.BFAQ_IDX}");
    formData.append("category", category);
    formData.append("question", question);
    formData.append("answer", answer);

    $.ajax({
        url: "/boardFaq/updateFaqAjax",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
