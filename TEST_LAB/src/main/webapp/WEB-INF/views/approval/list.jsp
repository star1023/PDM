...
$(document).ready(function(){
			...
			success:function(data) {
				...
				if( data.totalCount > 0 ) {
					...
					data.approvalList.forEach(function (item) {
						...
						html += "			<a href=\"#\" onclick=\"approvalInfo('"+item.apprNo+"', '"+item.tbType+"', '"+item.type+"', '"+item.tbKey+"','my'); return false;\">"+item.title+"</a>";
						html += "		</td>";
						html += "		<td>"+nvl(item.currentUserName,'')+"</td>";
						html += "		<td>"+item.modDate+"</td>";
						html += "		<td><ul class=\"list_ul\">";
						if( item.type == '0' ) {
							if( item.lastState == '0' ) {
								html += "		<li><button class=\"btn_doc\" onClick=\"deleteAppr('"+item.apprNo+"','"+item.tbType+"','"+item.tbKey+"')\"><img src=\"/resources/images/icon_doc06.png\"> 상신취소</button></li>";	
							} //else if( item.lastState == '2' && item.lastApprNo == '0' && item.apprNo == item.maxApprNo) {
							else if( item.lastState == '2') {	
								html += "		<li><button type=\"button\" class=\"btn_doc\" onClick=\"reAppr('"+item.apprNo+"','"+item.tbType+"','"+item.tbKey+"');\"><img src=\"/resources/images/icon_doc03.png\"> 재상신</button></li>";
							}
						}
						//html += "				<li><button class=\"btn_doc\" onClick=\"viewApprInfo('"+item.apprNo+"')\"><img src=\"/resources/images/icon_doc03.png\">결재상태</button></li>";
						html += "			</ul>";
						html += "		</td>";
						html += "	</tr>	";		
					});
	...
	function deleteAppr( apprNo, tbType, tbKey ) {		
		var URL = "../approval/deleteApprAjax";
		$.ajax({
			type:"POST",
			url:URL,
			data:{
	...
	function reAppr( apprNo, tbType, tbKey ) {		
	...
	function viewApprInfo( apprNo ) {
		document.location.href="#open3";
		var URL = "../approval/apprInfoAjax";
		$.ajax({
			type:"POST",
			url:URL,
			data:{
			...
			success:function(data) {
				...
				//referenceList
				$("#apprTxt").html("");
				var apprTxt = data.apprItemHeader.tbTypeName;
				if( data.apprItemHeader.type == '3' ) {
					apprTxt += "프린트 및 다운로드 ";
				}
				apprTxt += "결재";
				$("#apprTxt").html(apprTxt);
				var html="";
				$("#apprList").html(html);
				data.apprItemList.forEach(function (item) {
					html += "<li>";
					if( item.seq == '1' ) {
						html +=	"	<span>상신</span> "+item.userName;
					} else {
						html +=	"	<span>"+(eval(item.seq)-1)+"차결재</span> "+item.userName;
					}
					html += " 	<strong> "+item.authName+"/"+item.deptCodeName+"&nbsp;";
					html += "	(</strong><i> "+item.stateText+"</i><strong>";
					html += " 	:2019:01:12 23:23 ";
					html += " 	)</strong>";
					if( item.note != '' ) {
						html += " 	의견 <img src=\"/resources/images/icon_app_mass.png\"/>";
					}
					html += "</li>";
				});
				$("#apprList").html(html);
				...
				var refListHtml="";
				var circListHtml="";
				$("#refList").html(refListHtml);
				$("#circList").html(circListHtml);
				data.referenceList.forEach(function (item) {
					if( item.type == 'R' ) {
						refListHtml += "<li><span> 참조</span> "+item.userName+" <strong> "+item.authName+"/"+item.deptCodeName+"</strong></li>";
					} else if( item.type == 'C' ) {
						circListHtml += "<li><span> 회람</span> "+item.userName+" <strong> "+item.authName+"/"+item.deptCodeName+"</strong></li>";
					} 
				});
				$("#refList").html(refListHtml);
				$("#circList").html(circListHtml);
...
<div class="wrap_in" id="fixNextTag">
	...
	<!-- 상세 페이지  start-->
		<h2 style="position:relative"><span class="title_s">Approval Doc</span>
			<span class="title">내가 올린 결재문서</span>
...
<div class="white_content" id="open3">
	<div class="modal" style="	width: 1100px;margin-left:-550px;height: 620px;margin-top:-200px;">
		...
		<div class="title6">
			<span class="txt" id="apprTxt">프린트 및 다운로드 결재</span>
		</div>
		<div class="main_tbl">
			<table class="insert_proc01 tbl_app">
				...
				<tbody>
					<tr>
						...
						<td colspan="3" id="">프린트 때문에 결재가 필요해요 해주세요</td></tr>
					<tr>
