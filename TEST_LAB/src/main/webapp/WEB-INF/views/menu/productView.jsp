...
<script type="text/javascript">
	...
	function fn_versionUp(idx) {
		location.href = '/menu/versionUpProductForm?idx='+idx;
	}
	...
	function fn_loadApprovalLine() {
		var URL = "../menu/selectApprovalLineAjax";
		$.ajax({
			type:"POST",
			url:URL,
			data:{
	...
	function fn_apprLineSave(obj){
		...
		if( !chkNull($("#apprLineName").val()) ) {
			...
			if( $("#apprLine option").length == 0 ) {
				...
				formData.append("apprLine", $("#apprLine").selectedValues());
				formData.append("docType", "PROD");
				var URL = "../menu/insertApprLineAjax";
				$.ajax({
					type:"POST",
					url:URL,
					dataType:"json",
	...
	function fn_changeApprLine(obj) {
		...
		if( $("#apprLineSelect").selectedValues()[0] != "" ) {
			var URL = "../menu/selectApprovalLineItemAjax";
			$.ajax({
				type:"POST",
				url:URL,
				data:{
	...
	function fn_deleteApprLine(obj){
		if( $("#apprLineSelect").selectedValues()[0] != "" ) {
			var URL = "../menu/deleteApprLineAjax";
			$.ajax({
				type:"POST",
				url:URL,
				data:{
