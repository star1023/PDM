...
<script type="text/javascript">
	...
	function fn_insert(){
		...
		if( !chkNull($("#title").val()) ) {
			...
			//기존 데이터 확인
			var URL = "../menu/selectProductCountAjax";
			$.ajax({
				type:"POST",
				url:URL,
				data:{
	...
	function fn_search() {
		/*
		if( !chkNull($("#searchValue").val()) ) {
			alert("제품코드 또는 제품명을 입력해주세요.");
			$("#searchValue").focus();
			return;
		} else {
			var URL = "../menu/selectSearchProductAjax";
			$.ajax({
				type:"POST",
				url:URL,
				data:{
	...
	function fn_copy(idx) {
		console.log(idx);
		var URL = "../menu/selectProductDataAjax";
		$.ajax({
			type:"POST",
			url:URL,
			data:{
