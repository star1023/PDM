...
<script type="text/javascript">
	...
	var mfgProcessDocDispEdit_Data = {Body:[${Disp_JsonData}]};

	$(document).ready(function(){
		Grids.OnReady = function(grid){
			if(grid.id == "mfgProcessDocDispEdit"){
				GridFunctions.Ready = true;
			}
		};
	...
	GridFunctions.addGridRow = function(gridId){
		if(gridId == "mfgProcessDocDispEdit"){
			if($('input[name=isAutoDisp]').val() == "1"){return;}
			Grids.mfgProcessDocDispEdit.AddRow(null, null, 1, true);
		}
	}
	GridFunctions.removeGridRow = function(gridId){
		if(gridId == "mfgProcessDocDispEdit"){
			//if($('input[name=isAutoDisp]').val() == "1"){return;}
			var rows = Grids.mfgProcessDocDispEdit.Rows;
			$.each(rows,function(index,row){
				if(row.Kind == "Data" && row.Deleted){
					Grids.mfgProcessDocDispEdit.RemoveRow(row);
				}
			});
		}
	}
	GridFunctions.removeAllRows = function(gridId){
		console.log("GridFunctions.removeAllRows: gridId=" + gridId);
		if(gridId == "mfgProcessDocDispEdit"){
			for(var row = Grids.mfgProcessDocDispEdit.GetFirst(); row; row = Grids.mfgProcessDocDispEdit.GetNext(row)){
				Grids.mfgProcessDocDispEdit.DeleteRowT(row,2);
			}
			GridFunctions.removeGridRow(gridId);
		}
	}
	GridFunctions.Ready = false;
	GridFunctions.dispArray = [];
	GridFunctions.doDisp = function(){
		window.setTimeout(function(){
			if(GridFunctions.Ready){
				GridFunctions.removeAllRows("mfgProcessDocDispEdit");

				GridFunctions.dispArray.forEach(function(v){
					...
					var newRow = Grids.mfgProcessDocDispEdit.AddRow(null, null, 1, true);

					Grids.mfgProcessDocDispEdit.SetValue(newRow, "MATNAME", v.NAME, 1);
					Grids.mfgProcessDocDispEdit.SetValue(newRow, "EXCRATE", round(excRate,3), 1);
					Grids.mfgProcessDocDispEdit.SetValue(newRow, "INCRATE", round(incRate,3), 1);

				})
			}else{
				GridFunctions.doDisp();
			}
		},100);
	}
	GridFunctions.SetCanEdit = function(gridId,CanEdit){
		if(gridId == "mfgProcessDocDispEdit"){
			var rows = Grids.mfgProcessDocDispEdit.Rows;
			$.each(rows,function(index,row){
				if(row.Kind == "Data"){
					Grids.mfgProcessDocDispEdit.SetAttribute(row, "EXCRATE", "CanEdit", CanEdit, 1);
					Grids.mfgProcessDocDispEdit.SetAttribute(row, "INCRATE", "CanEdit", CanEdit, 1);
				}
			});
		}
	}
		...
		<div class="group01 mt20">
			<c:if test="${mfgProcessDoc.calcType == '10'}">
				<c:if test="${mfgProcessDoc.companyCode != 'MD'}">
					<c:set var="calcTypeText" value="일반제품"/>
				</c:if>
				<c:if test="${mfgProcessDoc.companyCode == 'MD'}">
					<c:set var="calcTypeText" value="프리믹스"/>
				</c:if>
			</c:if>
			...
			<!-- 기준정보 start --------------------------------------------------------------------------------------------------------------------------------------------------------->
			<div class="title5"><span class="txt">01. '${productName}' 기준정보 (${calcTypeText})</span></div>
			<div class="main_tbl">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div class="selectbox  ml5 req" style="width:100%">
		                            <!-- <label for="mfg_plant_select">--공장선택--</label>
		                            <select id="mfg_plant_select" name="plantCode" onchange="changeMfgPlant(event,'mfg_plantline_select')">
		                            ...
		                            <select id="mfg_plant_select" name="plantCode" onchange="plantChange(event,'mfg_plantline_select')">
						...
						<tr>
							...
							<td>
								...
								<c:if test="${userUtil:getUserGrade(pageContext.request) != '3'}">
									<input type="hidden" name="stlal" style="width: 100%;" placeholder="BOM담당자만 입력" value="${mfgProcessDoc.stlal}" />
								</c:if>
							</td>
							<th>배합중량</th>
							<td>
								<input type="text" id="mixWeight" name="mixWeight" onkeyup="mixWeightKeyUp(event)" value="${mfgProcessDoc.mixWeight}" style="width: 30%;" class="req"/> kg
								( <input type="text" name="bagAmout" onkeyup="bagAmoutKeyUp(event)" value="${mfgProcessDoc.bagAmout}" style="width: 20%;" class="req" /> 포)
								<%-- 
								<c:if test="${productType == 10}">
									<input type="text" id="mixWeight" name="mixWeight" style="width: 30%;" class="req" value="${mfgProcessDoc.mixWeight}" /> kg
									( <input type="text" name="bagAmout" style="width: 20%;" class="req" value="${mfgProcessDoc.bagAmout}" /> 포)
								</c:if>
								<c:if test="${productType != 10}">
									<input type="text" id="mixWeight" name="mixWeight" style="width: 30%;" class="read_only" readonly="readonly" value="${mfgProcessDoc.mixWeight}" /> kg
									( <input type="text" name="bagAmout" style="width: 20%;" class="read_only" readonly="readonly" value="${mfgProcessDoc.bagAmout}" /> 포)
								</c:if>
								 --%>
							</td>
							<th>BOM수율</th>
			...
			<%-- <div class="tab03" style="position: relative;">
				<div style="display: inline-block; position: absolute; left: 0">11</div>
				<div style="display: inline-block;">
					<ul style="white-space: nowrap; overflow: auto;">
						...
						<c:forEach items="${mfgProcessDoc.sub}" var="sub" varStatus="subStatus">
							...
							<a href="javascript:;" id="subProdAddBtn_${subStatus.index}"><li class="${liClass}" onclick="changeTab(this)">
								<input type="text" name="subProdName" style="width: 180px;" placeholder="부속제품명 입력" class="req" value="${sub.subProdName}" />
								<button name="subProdDelBtn" class="tab_btn_del"  onclick="removeSubProduct('${subStatus.index}')"><img src="/resources/images/btn_table_header01_del02.png"></button>
							</li></a>
						</c:forEach>
						<!-- 부속제품추가됬으나 감춰진탭 -->
						<a id="addSubProdA" href="javascript:;"><li class="" onclick="addSubProduct(this)">
							<button class="tab_btn_add"><img src="/resources/images/btn_pop_add.png"></button>
						</li></a>
						<!-- 부속제품 추가시 -->
					</ul>
				</div>
				<div style="display: inline-block; position: absolute; right: 0">33</div>
			</div>
						...
						<table class="insert_proc01" style="border-top: none; border-right: 1px solid #d1d1d1">
							...
							<tbody>
								<tr>
									...
									<c:if test="${mfgProcessDoc.calcType == '10'}">
										<th>배합 총합</th>
										<td><input type="text" name="subProdBomRateTotal" style="width: 100%;" value="${sub.subProdBomRateTotal}" readonly="readonly" class="read_only" /></td>
									</c:if>
									<th>기준수량</th>
									<td>
										<c:if test="${mfgProcessDoc.calcType == '20'}">
											<input type="text" name="subProdStdAmount" style="width: 100%;" class="req"  onkeyup="setStdAmount(event)" value="${sub.stdAmount}"/>
										</c:if>
										<c:if test="${mfgProcessDoc.calcType != '20'}">
									...
									<table class="tbl05">
										...
										<tbody id="mixTbody_${subStatus.index}_${baseStatus.index}">
											...
											<c:forEach items="${mix.item}" var="item" varStatus="itemStatus">
												...
												<c:if test="${item.unit == 'KG' or item.unit == 'L'}">
												...
												<Tr id="mixRow_${subStatus.index}_${baseStatus.index}_${itemStatus.index}" class="temp_color" style="background-color: ${bgColor}">
													...
													<Td>
														...
														<input type="text" name="itemSapCode" style="width: 100px" class="req code_tbl" onkeyup="checkMaterail(event, 'B', '${item.itemCode}')" value="${item.itemCode}"/>
													...
													<Td>
														<c:choose>
															<c:when test="${item.unit == 'KG' or item.unit == 'L'}">
																<c:set var="readOnlyClass" value="read_only"/>
																<c:set var="readOnly" value='readonly="readonly"'/>
															</c:when>
															<c:otherwise>
																<c:set var="readOnlyClass" value="req"/>
																<c:set var="readOnly" value=''/>
															</c:otherwise>
														</c:choose>
														<input type="text" name="itemBomRate" style="width: 100%" class="${readOnlyClass}" ${readOnly} onkeyup="bomRateKeyup(this)" value="${item.bomRate}" />
													</Td>
										...
										<tfoot>
											<Tr>
												<Td colspan="3">합계</Td>
												<Td><input type="text" name="bomAmountTotal" style="width: 100%" readonly="readonly" class="read_only" value="<fmt:formatNumber value="${bomAmountTotal}" pattern="0.###"/>"/></Td>
												...
												<Td>&nbsp;</Td>
												<Td>&nbsp;</Td>
												<%-- <Td><input type="text" name="itemWeightTotal" style="width: 100%" readonly="readonly" class="read_only" value="<fmt:formatNumber value="${weightTotal}" pattern="0.###"/>"/></Td> --%>
												<Td>&nbsp;</Td>
												<Td>&nbsp;</Td>
												<Td>&nbsp;</Td>
											</Tr>
								...
								<div class="nomal_mix">
									<div class="table_header02">
										...
										<span class="table_order_btn">
											...
											type="text" name="divWeight" style="width: 50px" value="${cont.divWeight}" onkeyup="divWeightKeyup(event)"/> g <input
									...
									<table class="tbl05">
										...
										<tbody id="contentTbody_${subStatus.index}_${baseStatus.index}">
											<c:set var="bomRateTotal" value="0"/>
											<c:set var="bomAmountTotal" value="0"/>
											<c:forEach items="${cont.item}" var="item" varStatus="itemStatus">
												...
												<c:if test="${itemStatus.index == 0}">
													<c:set var="firstItemUnit" value="${item.unit}"/>
												</c:if>
												<c:if test="${item.unit == 'KG' or item.unit == 'L'}">
													<c:set var="bomRateTotal" value="${bomRateTotal + item.bomRate}"/>
												</c:if>
												<c:set var="bomAmountTotal" value="${bomAmountTotal + item.bomAmount}"/>
												<c:set var="weightTotal" value="${weightTotal + item.weight}"/>
												<Tr id="contentRow_${subStatus.index}_${baseStatus.index}_${itemStatus.index}" class="temp_color" style="background-color: ${bgColor}">
													<Td><input type="checkbox" id="content_${subStatus.index}_${baseStatus.index}_${itemStatus.index}"><label for="content_${subStatus.index}_${baseStatus.index}_${itemStatus.index}"><span></span></label></Td>
													<Td>
														<input type="hidden" name="itemImNo" style="width: 100px" class="req code_tbl" value="${item.itemImNo}"/>
														<input type="text" name="itemSapCode" style="width: 100px" class="req code_tbl" onkeyup="checkMaterail(event, 'B', '${item.itemCode}')" value="${item.itemCode}"/>
														<button class="btn_code_search2" onclick="openMaterialPopup(this, 'B')"></button>
													...
													<Td><input type="text" name="itemName" style="width: 85%" readonly="readonly" class="read_only" value="${item.itemName}"/><button class="btn_code_info2"></button></Td>
													<Td><input type="text" name="itemBomAmount" style="width: 100%" class="req" onkeyup="calcBomAmountCont(this)" value="${item.bomAmount}"/></Td>
													<Td>
														<c:choose>
															<c:when test="${item.unit == 'KG' or item.unit == 'L'}">
																<c:set var="readOnlyClass" value="read_only"/>
														...
														<input type="text" name="itemBomRate" style="width: 100%" class="${readOnlyClass}" onkeyup="bomRateKeyup(this)" readonly="${readOnly}" value="${item.bomRate}"/>
													</Td>
													<Td><input type="text" name="itemUnit" style="width: 100%" class="readonly" readonly="readonly" value="${item.unit}"/></Td>
													<Td><input type="text" name="itemWeight" style="width: 100%" class="req" onkeyup="caclWeight(this)" value="${item.weight}"/></Td>
													<Td><input type="text" name="itemPosnr" style="width: 100%" value="${item.posnr}"/></Td>
													...
													<Td>
														...
														<div class="selectbox" style="width: 90%">
															<select name="itemStorageCode" id="select_storage_content_${itemStatus.index}" ${disabled}>
																<c:set var="itemStorageCodeText" value=""/>
																<option value="">선택</option>
																<c:forEach items="${specificStorageList}" var="storage">
																	<c:set var="selected" value="${storage.storageCode == item.storageCode ? 'selected' : ''}"/>
															...
															<c:set var="itemStorageCodeText" value="${itemStorageCodeText == '' ? '선택' : itemStorageCodeText}"/>
															<label for="select_storage_content_${itemStatus.index}">${itemStorageCodeText}</label>
														</div>
													</Td>
												</Tr>
											</c:forEach>
										...
										<tfoot>
											<Tr>
												<Td colspan="3">합계</Td>
												<Td><input type="text" name="bomAmountTotal" style="width: 100%" readonly="readonly" class="read_only" value="<fmt:formatNumber value='${bomAmountTotal}' pattern='0.###'/>"/></Td>
												<%-- <c:if test="${firstItemUnit != 'KG' and firstItemUnit != 'L' }">
													<c:set var="bomRateTotal" value="0"/>
												</c:if> --%>
												<Td><input type="text" name="bomRateTotal" style="width: 100%" readonly="readonly" class="read_only" value="<fmt:formatNumber value='${bomRateTotal}' pattern='0.###'/>"/></Td>
												<Td>&nbsp;</Td>
												<Td>&nbsp;</Td>
			...
			<div class="title5" style="float: left; margin-top: 30px;">
				<span class="txt">
					03. 표시사항 배합비
					<ul class="list_ul3">
						<li><a href="javascript:changeAutoDisp(0)">직접입력</a></li><li><a href="javascript:changeAutoDisp(1)">자동계산</a></li>
					</ul>
					<input type="hidden" name="isAutoDisp" value=""/>
				</span>
			</div>

			<div class="fl-box panel-wrap" style="border-top:2px solid #4b5165;">
				<bdo Debug=""
					 Layout_Url="/dev/mfgProcessDocDispLayout?gridId=mfgProcessDocDispEdit&edit=1"
<%--						 Data_Url="/AdminReport/userListData"--%>
					Data_Script="mfgProcessDocDispEdit_Data" >
				</bdo>
			</div>
			...
			<table class="tbl05">
				...
				<tbody id="packageTbody_0" name="packageTbody">
					<c:forEach items="${mfgProcessDoc.pkg}" var="pkg" varStatus="pkgStatus">
						<c:set var="bgColor" value="#fff" />
						<c:if test='${fn:indexOf(pkg.itemName, "[임시]") >= 0}'>
							<c:set var="bgColor" value="#ffdb8c" />
						</c:if>
						<Tr id="pkgRow_${pkgStatus.index}" class="temp_color" style="background-color: ${bgColor}">
							...
							<Td>
								...
								<input type="text" name="itemSapCode" style="width: 100px" class="req code_tbl" onkeyup="checkMaterail(event, 'R', '${pkg.itemCode}')" value="${pkg.itemCode}" />
							...
							<td><input type="text" name="itemBomAmount" style="width:80%" class="req code_tbl" onkeyup="clearNoNum(this)" value="${pkg.bomAmount}"><button class="btn_code_search3" onclick="openUnitDialog(event)"></button></td>
							<Td>
							...
							<td><input type="text" name="itemBomRate" style="width:100%" class="req" onkeyup="clearNoNum(this)" value="${pkg.bomRate}" ></td>
							<Td><input type="text" name="itemUnit" style="width: 100%" class="readonly" readonly="readonly" value="${pkg.unit}" /></Td>
							...
							<Td>
								<c:choose>
									<c:when test="${pkg.itemCode == 'P001' || pkg.itemCode == 'P10001' || pkg.itemCode == 'P10002' || pkg.itemCode == 'P10003'}">
										<c:set var="disabled" value="disabled"/>
									</c:when>
									<c:otherwise>
										<c:set var="disabled" value=""/>
								...
								<div class="selectbox" style="width: 90%">
									<select name="itemStorageCode" id="select_storage_package_${pkgStatus.index}" ${disabled}>
										<c:set var="itemStorageCodeText" value=""/>
										<option value="">선택</option>
										<c:forEach items="${specificStorageList}" var="storage">
											<c:set var="selected" value="${storage.storageCode == pkg.storageCode ? 'selected' : ''}"/>
											<c:if test="${storage.storageCode == pkg.storageCode}">
												<c:set var="itemStorageCodeText" value="${storage.storageCode}"/>
											</c:if>
											<option value="${storage.storageCode}" ${selected}>${storage.storageCode}</option>
										</c:forEach>
									...
									<c:set var="itemStorageCodeText" value="${itemStorageCodeText == '' ? '선택' : itemStorageCodeText}"/>
									<label for="select_storage_package_${pkgStatus.index}">${itemStorageCodeText}</label>
								</div>
							</Td>
						</Tr>
					</c:forEach>
				...
				<table class="insert_proc01">
					...
					<tbody>
						...
						<tr>
							...
							<%-- <th>관리중량</th>
							<td>
								<input type="text" name="adminWeightFrom" style="width: 20%; float: left" class="read_only" readonly="readonly" value="${mfgProcessDoc.adminWeightFrom}"/>
								...
								<input type="text" id="adminWeightTo" name="adminWeightTo'" style="width: 20%; float: left" class="read_only" readonly="readonly" value="${mfgProcessDoc.adminWeightTo}"/>
						...
						<tr>
							...
							<td>
								...
								<span style="float: left">&nbsp;~&nbsp;</span>
								<input type="text" id="adminWeightTo" onkeyup="changeWeight(event)" name="adminWeightTo'" style="width: 20%; float: left" class="req" value="${mfgProcessDoc.adminWeightTo}"/>
								<%-- <input type="text" name="adminWeightUnitTo" style="width: 20%; float: left; margin-left: 2px" class="read_only" readonly="readonly" value="${mfgProcessDoc.adminWeightUnitTo}"/> --%>
								<div class="selectbox req ml5" style="width: 15%">
			...
			<c:if test="${userUtil:getIsAdmin(pageContext.request) == 'Y' || userUtil:getUserGrade(pageContext.request) == '3'}">
				...
				<table class="tbl05">
					...
					<tbody id="consumeTbody_0" name="consumeTbody">
						<c:forEach items="${mfgProcessDoc.cons}" var="cons" varStatus="consStatus">
							<c:set var="bgColor" value="#fff" />
							<c:if test='${fn:indexOf(cons.itemName, "[임시]") >= 0}'>
								<c:set var="bgColor" value="#ffdb8c" />
							</c:if>
							<Tr id="consumeRow_${consStatus.index}" style="background-color: ${bgColor}">
								...
								<Td>
									...
									<input type="text" name="itemSapCode" style="width: 100px" class="req code_tbl" onkeyup="checkMaterail(event, '%', '${cons.itemCode}')" value="${cons.itemCode}"/>
								...
								<Td><input type="text" name="itemBomRate" style="width: 100%" class="req" onkeyup="clearNoNum(this)" value="${cons.bomRate}"/></Td>
								<Td><input type="text" name="itemUnit" style="width: 100%" class="readonly" readonly="readonly" value="${cons.unit}" /></Td>
								...
								<Td>
									<c:choose>
									<c:when test="${cons.itemCode == 'P001' || cons.itemCode == 'P10001' || cons.itemCode == 'P10002' || cons.itemCode == 'P10003'}">
										<c:set var="disabled" value="disabled"/>
									</c:when>
									<c:otherwise>
										<c:set var="disabled" value=""/>
									...
									<div class="selectbox" style="width: 90%">
										<select name="itemStorageCode" id="select_storage_consume_${consStatus.index}" ${disabled}>
											<c:set var="itemStorageCodeText" value=""/>
											<option value="">선택</option>
											<c:forEach items="${specificStorageList}" var="storage">
												<c:set var="selected" value="${storage.storageCode == cons.storageCode ? 'selected' : ''}"/>
												<c:if test="${storage.storageCode == cons.storageCode}">
													<c:set var="itemStorageCodeText" value="${storage.storageCode}"/>
												</c:if>
												<option value="${storage.storageCode}" ${selected}>${storage.storageCode}</option>
											</c:forEach>
										...
										<c:set var="itemStorageCodeText" value="${itemStorageCodeText == '' ? '선택' : itemStorageCodeText}"/>
										<label for="select_storage_consume_${consStatus.index}">${itemStorageCodeText}</label>
									</div>
								</Td>
							</Tr>
						</c:forEach>
				...
				<table class="insert_proc01">
					...
					<tbody name="specTbody">
						...
						<tr>
							...
							<td>크기</td>
							<td colspan="4"><input type="text" name="size" style="width: 100%" value="${strUtil:getHtmlBr(spec.size)}"/></td>
						...
						<tr>
							<th style="border-left: none;">성상</th>
							<td>토핑,착색,흐름성 </td>
							<td colspan="4"><input type="text" name="feature" value="${spec.feature}" style="width: 100%"/></td>
							<td></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="4"><input type="text" name="noodlesAcidity" value="${spec.noodlesAcidity}" style="width: 100%" /></td>
							<td></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="4"><input type="text" name="deoxidizer" value="${spec.deoxidizer}" style="width: 100%" /></td>
							<td></td>
						</tr>
						<tr>
							<th style="border-left: none;" colspan="2">질소충전제품</th>
							<td colspan="4"><input type="text" name="nitrogenous" style="width: 100%" value="${spec.nitrogenous}"/></td>
							<td></td>
						</tr>
					</tbody>
				</table>
					...
					<dd style="width: 80%">
						<div class="selectbox req" style="width:25%">
                            ...
                            <select id="selectUnitType" name="changeUnitType" onchange="changeConvertType(event)">
                            	...
                            	<!-- 삭제요청 S201013-00009 아래와 같이 삭제 요청 받았다가 다시 생성 요청 받음 S220511-00011 -->
...
<div id=btn_temp class="tab03">
	<ul>
		<li id="subProdAddBtn_temp" class="select" onclick="changeTab(this)" ondblclick="changeSubProdName(event)">
			<span class="unselectable" ondblclick="dblSubProdNameSpan(event)">더블클릭하여 부속제품명 입력</span>
		...
		<div class="tbl_in_con">
			...
			<div id="mixDiv_temp" class="nomal_mix">
				...
				<table class="tbl05">
					...
					<tbody id="mixTbody_temp">
						<Tr id="mixRow_temp" class="temp_color">
							<Td><input type="checkbox" id="mix_temp"><label for="mix_temp"><span></span></label></Td>
							<Td>
							...
							<Td><input type="text" name="itemBomAmount" style="width: 100%" class="req"  onkeyup="calcBomAmountMix(this)"/></Td>
		...
		<div class="tbl_in_con">
			...
			<div id="contDiv_temp" class="nomal_mix">
				...
				<table class="tbl05">
					...
					<tbody id="contentTbody_temp">
						<Tr id="contentRow_temp" class="temp_color">
							<Td><input type="checkbox" id="content_temp"><label for="content_temp"><span></span></label></Td>
							<Td>
								<input type="hidden" name="itemImNo" style="width: 100px" class="req code_tbl"/>
								<input type="text" name="itemSapCode" style="width: 100px" class="req code_tbl" onkeyup="checkMaterail(event, 'B', '${item.itemCode}')"/>
								<button class="btn_code_search2" onclick="openMaterialPopup(this, 'B')"></button>
							...
							<Td>
								<!-- <input type="text" name="itemStorageCode" style="width: 100%; cursor: pointer" class="read_only" readonly="readonly" onclick="openStorageDialog(event, 'dialog_storage')"/> -->
								<div class="selectbox" style="width: 90%">
									<select name="itemStorageCode" id="select_storage_content_temp">
										<option value="" selected>선택</option>
										<c:forEach items="${specificStorageList}" var="sotrage">
											<option value="${sotrage.storageCode}">${sotrage.storageCode}</option>
										</c:forEach>
									</select>
									<label for="select_storage_content_temp">선택</label>
								</div>
							</Td>
						</Tr>
					</tbody>
...
<table id="consTable_temp">
	<Tr id="consumeRow_temp">
		<Td><input type="checkbox" id="consume_1"><label for="consume_1"><span></span></label></Td>
		<Td>
			<input type="hidden" name="itemImNo" style="width: 100px" class="req code_tbl"/>
			<input type="text" name="itemSapCode" style="width: 100px" class="req code_tbl" onkeyup="checkMaterail(event, '%', '${item.itemCode}')"/>
			<button class="btn_code_search2" onclick="openMaterialPopup(this, '')"></button>
		</Td>
		<Td><input type="text" name="itemName" style="width: 85%" readonly="readonly" class="read_only" /><button class="btn_code_info2"></button></Td>
		<Td><input type="text" name="itemBomRate" style="width: 100%" class="req" onkeyup="clearNoNum(this)"/></Td>
		...
		<Td>
			<!-- <input type="text" name="itemStorageCode" style="width: 100%; cursor: pointer" class="read_only" readonly="readonly" onclick="openStorageDialog(event, 'dialog_storage')"/> -->
			<div class="selectbox" style="width: 90%">
				<select name="itemStorageCode" id="select_storage_consume_temp">
					<option value="" selected>선택</option>
					<c:forEach items="${specificStorageList}" var="sotrage">
						<option value="${sotrage.storageCode}">${sotrage.storageCode}</option>
					</c:forEach>
				</select>
				<label for="select_storage_consume_temp">선택</label>
			</div>
		</Td>
	</Tr>
</table>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		subProdTabBtn = '<li id="" class="select" onclick="changeTab(this)" ondblclick="changeSubProdName(event)">' + $('#subProdAddBtn_temp').html() + '</li>';
		...
		var regGubun = '${mfgProcessDoc.regGubun}';
		if(regGubun.length > 0){
			$('#select_regGubun option[value='+regGubun+']').prop('selected', true);
			$('#select_regGubun').change();
		}
		
		var isAutoDisp = '${mfgProcessDoc.isAutoDisp}'
		var isOnload = true;
		...
		$('textarea[name=memo]').val(restoreStr(restoreStrAmp(memo)));
		
		var menuProcess = $('textarea[name=menuProcess]').val();
		$('textarea[name=menuProcess]').val(restoreStr(restoreStrAmp(menuProcess)));
		
		var standard = $('textarea[name=standard]').val();
		$('textarea[name=standard]').val(restoreStr(restoreStrAmp(standard)));
		...
		//doDisp(false);
		//setDispAttr(isAutoDisp, isOnload);
		changeAutoDisp(isAutoDisp,isOnload);
	});
		...
		$.ajax({
			...
			success: function(data){
				...
				if(materailList.length == 1){
					...
					if(rowId.indexOf('cons')<0){
						if(item.unit == 'KG' || item.unit == 'L'){
							$('#'+rowId + ' input[name=itemBomRate]').attr('readonly', true);
							$('#'+rowId + ' input[name=itemBomRate]').attr('class', 'read_only');
							$('#'+rowId + ' input[name=itemBomAmount]').keyup();
						} else {
							$('#'+rowId + ' input[name=itemBomRate]').attr('readonly', false);
							$('#'+rowId + ' input[name=itemBomRate]').attr('class', 'req');
						}
					}
	...
	function addSubProduct(element){
		...
		setUniqueId('div', 'mixDiv_temp');
		setUniqueId('div', 'contDiv_temp');
	}
	...
	function addRow(element, type){
		...
		if(calcType == 30){
			$('#'+ type + 'Row_' + tBodyNumber + '_' + randomId).children('td').children('input[name=itemBomRate]').attr('readonly', false);
			$('#'+ type + 'Row_' + tBodyNumber + '_' + randomId).children('td').children('input[name=itemBomRate]').attr('class', 'req');
		}
	...
	function removeRow(element){
		...
		var headerId = $(element).parent().parent().attr('id');
		if(headerId == 'dispHeaderDiv'){
			calcDisp();
		}
	...
	function calcAll(){
		...
		doDisp(false);
		setDispAttr($('input[name=isAutoDisp]').val(), false);
	}
	...
	/* 원료 배합의 총합 - 240724 */
	function calcBomRateTotalAll(){
		var bomRateTotalAll = 0;
		var calcType = '${mfgProcessDoc.calcType}';
		...
		if(calcType == "10"){
			...
			$('div[name=subProdDiv]').toArray().forEach(function(sub, subIndex){
				// 배합비 BOM합계 합
				var mixDiv = $(sub).children('div:last').children('div:nth-child(2)');
				var mixBodyFoot = mixDiv.children('div.nomal_mix').children('table').children('tfoot');
				mixBodyFoot.toArray().forEach(function(tBody, tBodyIndex){
					
					var bomRateTotalMix = $(tBody).children('tr').children('td').children('input[name=bomRateTotal]').val();
					bomRateTotalAll += Number(bomRateTotalMix);
				});
				
				// 내용물 BOM합계 합
				var contDiv = $(sub).children('div:last').children('div:nth-child(4)');
				var contBodyFoot = contDiv.children('div.nomal_mix').children('table').children('tfoot');
				contBodyFoot.toArray().forEach(function(tBody, tBodyIndex){
				
					var bomRateTotalcont = $(tBody).children('tr').children('td').children('input[name=bomRateTotal]').val();
					bomRateTotalAll += Number(bomRateTotalcont);
				});
				...
				var $targetInput = $(sub)
						...
						.find('input[name="subProdBomRateTotal"]');
				
				$targetInput.val(round(bomRateTotalAll,3));
			});
				
			console.log("bomRateTotalAll :: "+ bomRateTotalAll);
			
			
			// 다른 방법 참고
			//$('input[name=bomRateTotal]').toArray().forEach(function(bomRateTotal){		
			//	console.log("bomRateTotal :: " +Number($(bomRateTotal).val()));
			//	bomRateTotalAll2 += Number($(bomRateTotal).val());
			//});
		}else{
			console.log("기준 10 이외");	
	...
	function calcBomAmountMix(element){
		clearNoNum(element);
		$(element).parent().parent().children('td').children('input[name=itemWeight]').val($(element).val()); //중량
		...
		var subProdDivWeight = Number($(subProdHead).children().children('tbody').children().children('td').children('input[name=subProdDivWeight]').val()); //분할중량
		var subProdStdAmount = Number($(subProdHead).children().children('tbody').children().children('td').children('input[name=subProdStdAmount]').val()); //기준수량
		
		var productBomRatio = $('input[name=bomRate]').val(); //bom수율
		var calcType = '${mfgProcessDoc.calcType}';
		...
		$('tbody[id^='+mixTbodyGroup+']').toArray().forEach(function(tbody){
		    ...
		    if(calcType != "30"){
		    	$(tbody).children('tr').toArray().forEach(function(tr){
		            ...
		            if(calcType == "10"){
		                ...
		                itemBomRate = round(subProdDivWeight * subProdStdAmount * (itemBomAmount / tableBakeryTotal) / 1000 / (productBomRatio / 100),3);
		            }
		            
		            itemBomRate = round(itemBomRate,3);
		    ...
		    totalRow.children('td').children('input[name=bomRateTotal]').val(round(bomRateTotal,3));
		    totalRow.children('td').children('input[name=bomAmountTotal]').val(round(bomAmountTotal,3));
	...
	function calcStdAmount(event){
		...
		//if(calcType != '30' && event.target.value == 0) return;
		...
		*/
		...
		var bomRate = Number($('#productBomRatio').val());
		...
		if(calcType != 20){ // !20
			if(calcType == 10){ // 10
				...
				if(isNaN(subProdStdAmount) || !isFinite(subProdStdAmount)) {
					subProdStdAmountElement.val('')
				} else {
				...
				//$('input[name=stdAmount]').val(Math.floor(subProdStdAmount));
			} else { // 30
				mix.toArray().forEach(function(mixHeader){
					var bomRateTotal = $(mixHeader).next().children('tfoot').children('tr').children('td').children('input[name=bomRateTotal]').val();
					mixBomRateTotal += Number(bomRateTotal);
				...
				var subProdStdAmount = mixBomRateTotal*(bomRate/100);
				
				var subProdDivWeightCream = nvl(subProdDivWeight, 0);
				if(subProdDivWeightCream != 0){
					subProdStdAmount = (mixBomRateTotal*1000*(bomRate/100)/subProdDivWeight).toFixed(1);
				}
				...
				//if(subProdStdAmount != Infinity) subProdStdAmountElement.val(Math.floor(subProdStdAmount));
				
				//$('input[name=stdAmount]').val(Math.floor(subProdStdAmount));
			}
		}
		
		var minimumStdAmount = 0;
		$('input[name=subProdStdAmount]').toArray().forEach(function(subProdStdAmt){
			if(minimumStdAmount == 0) {
				minimumStdAmount = Number($(subProdStdAmt).val());
			} else {
				if(minimumStdAmount > Number($(subProdStdAmt).val())){
					minimumStdAmount = Number($(subProdStdAmt).val());
				}
			}
		})
		...
		if(plantCode == '7100'){
			var stdAmountTotal = 0;
			$('input[name=subProdStdAmount]').toArray().forEach(function(subProdStdAmt){
				stdAmountTotal += Number($(subProdStdAmt).val());
			})
			
			var avgStdAmount = stdAmountTotal/$('input[name=subProdStdAmount]').length;
			$('input[name=stdAmount]').val(Math.floor(round(avgStdAmount,3)));
		} else {
			$('input[name=stdAmount]').val(Math.floor(minimumStdAmount));
		}
	}
	...
	function calcBomAmountCont(element){
		...
		var tableBakeryTotal = 0;
		$(element).parent().parent().parent().children('tr').toArray().forEach(function(tr){
			tableBakeryTotal += $(tr).children('td').children('input[name=itemBomRate]').val();
		})
		...
		if(calcType != "30"){
			...
			$(element).parent().parent().parent().children('tr').toArray().forEach(function(tr){
				...
				itemBomRate = round(subProdStdAmount * contDivWeight * (itemBomAmount/bomAmountTotal) / 1000,3) ;
				if((itemUnit == "KG" || itemUnit == "L")){
					$(tr).children('td').children('input[name=itemBomRate]').val(round(itemBomRate,3));
	...
	function calcDisp(e){
		...
		// })
		//
		// excRateTotal = round(excRateTotal,2);
		// incRateTotal = round(incRateTotal,2);
		//
	...
	function doDisp(isOnload){
		console.log("doDisp: isAutoDisp=" + $('input[name=isAutoDisp]').val()  + ", isOnload=" + isOnload);
		var calcType = '${mfgProcessDoc.calcType}';
		if($('input[name=isAutoDisp]').val() != '1'){
			// 자동계산이 아닌 경우
			return;
		...
		console.log(pecentArray);
		console.log(calcArray);

		setDispArrayToGrid(dispArray);

	}

	function setDispArrayToGrid(dispArray){
		if(!GridFunctions.Ready){window.setTimeout(function(){setDispArrayToGrid(dispArray)},200);return;}

		var isAutoDisp = $('input[name=isAutoDisp]').val();
		var gridId = "mfgProcessDocDispEdit";
		...
		dispArray.forEach(function(v){
			var newRow = grid.AddRow(null, null, 1, true);
			grid.SetValue(newRow, "MATNAME", v.NAME, 1);
			var excRate = v.EXCRATE;
			if(excRate=="0") {
				excRate = "";
			} else {
			...
			grid.SetValue(newRow, "EXCRATEAUTO", round(excRate,4), 1);
			grid.SetValue(newRow, "INCRATEAUTO", round(incRate,4), 1);

			if(isAutoDisp == "1"){
				grid.SetValue(newRow, "EXCRATE", round(excRate,4), 1);
				grid.SetValue(newRow, "INCRATE", round(incRate,4), 1);
			}
		});
	}
	...
	/* function setDisp(){
		...
		var totalContPortion = 1- mixPortion;
		...
		var totalContWeight = 0;
		allContBody.toArray().forEach(function(tbody){
			totalContWeight += Number($(tbody).parent().prev().children('span').children('input[name=divWeight]').val())
		})
		
		allContBody.toArray().forEach(function(tbody){
			var contWeight = Number($(tbody).parent().prev().children('span').children('input[name=divWeight]').val());
			var contPortion = totalContPortion*(contWeight/totalContWeight);
			console.log(contPortion);
	...
	function saveValid(){
		...
		var calcType = '${mfgProcessDoc.calcType}'
		if( calcType != '30' ) {
			if($('input[name=numBong]').val().length == 0){
				alert('봉당들이수를 입력하세요');
				$('input[name=numBong]').focus();
				return false;;
			}
			
			if($('input[name=numBox]').val().length == 0){
				alert('상자들이수를 입력하세요');
				$('input[name=numBox]').focus();
				return false;;
			}
		...
		/* if($('input[name=loss]').val().length == 0){
			alert('소성손실율를 입력하세요');
			$('input[name=loss]').focus();
			return false;
		} */
		...
		if($('tr[id^=mixRow]').length > 0){
			$('tr[id^=mixRow]').toArray().forEach(function(mixRow){
				...
				if(itemSapCode.length>0 || itemName.length>0 || itemBomAmount.length>0 || itemWeight.length>0 || itemBomRate>0){
		...
		if($('input[name=adminWeightFrom]').val().length == 0){
			alert('관리중량을 입력하세요');
			$('input[name=adminWeightFrom]').focus();
			return false;
		}
		
		if($('#adminWeightTo').val().length == 0){
			alert('관리중량을 입력하세요');
			$('input[name=adminWeightTo]').focus();
			return false;
		}
	...
	function updateMfgProcessDoc(state){
		...
		//state = nvl(state,'') != '7' ? '${mfgProcessDoc.state}' : state;
		//$('#lab_loading').show();
		...
		$.post('updateManufacturingProcessDoc', postData, function(data){
			if(data == 'S') {
				alert('문서 수정 완료');
	...
	function getPostData(state){
		...
		postData['docVersion'] = '${mfgProcessDoc.docVersion}';
		postData['state'] = state;
		postData['regUserId'] = '${mfgProcessDoc.regUserId}';
		postData['docType'] = 'N'; // N: 일반,  E: 엑셀
		postData['calcType'] = '${mfgProcessDoc.calcType}';
		postData['companyCode'] = '${mfgProcessDoc.companyCode}';
		...
		postData['lineProcessType'] = $('input[name=lineProcessType]').val();
		...
		$('div[name=subProdDiv]').toArray().forEach(function(subProd, subProdNdx){
			...
			postData['sub['+subProdNdx+'].subProdBomRateTotal'] = subProdHeadTbody.children('tr').children('td').children('input[name=subProdBomRateTotal]').val();
			...
			mixDiv.toArray().forEach(function(mix, mixNdx){
				...
				mixBody.children('tbody').children('tr').toArray().forEach(function(item, itemNdx){
					...
					if(sapCode.length>0 || itemName.length>0 || bomAmount.length>0 || itemWeight.length>0){
		...
		for(var row = Grids.mfgProcessDocDispEdit.GetFirst(); row; row = Grids.mfgProcessDocDispEdit.GetNext(row)){
			var dispPath = 'disp['+dispNdx+'].'
			postData[dispPath+'matName'] = row.MATNAME;
			postData[dispPath+'excRate'] = row.EXCRATE;
			postData[dispPath+'incRate'] = row.INCRATE;
			dispNdx++;
		}
	...
	function changeAutoDisp(isAuto,isOnload){
		console.log("changeAutoDisp isAuto=" + isAuto);
		...
		// }
		//var isOnload = false;
		if(isOnload == null) isOnload = false;
		doDisp(isOnload);
		setDispAttr(isAuto, isOnload);
	}
	
	function setDispAttr(isAuto, isOnload){
		if(!GridFunctions.Ready){window.setTimeout(function(){setDispAttr(isAuto, isOnload)},200);return;}

		$('input[name=isAutoDisp]').val(isAuto);

		Grids.mfgProcessDocDispEdit.Pasting = (isAuto == "1")?0:1;  // 복사 붙여넣기 공제
		...
		if(isAuto == 1){
			...
			GridFunctions.SetCanEdit("mfgProcessDocDispEdit",0);
		} else {
			...
			GridFunctions.SetCanEdit("mfgProcessDocDispEdit",1);
		}
	}
	...
	function plantChange(e, selectBoxId){
		var companyCode = '${mfgProcessDoc.companyCode}';
		...
		if( companyCode != '' && plantCode != '') {	
			$.ajax({
				...
				success:function(data) {
					...
					$("#"+selectBoxId).addOption("", "--생산라인선택--", false);
					$("#label_"+selectBoxId).html("--생산라인선택--");
					$.each(list, function( index, value ){ //배열-> index, value
