...
<script type="text/javascript">
	...
	GridFunctions.reloadGrid = function(gridId){
		...
		GridFunctions.Data.disp.forEach(function(disp, dispIndex){
			...
			Grids.mfgProcessDocDispEdit.SetValue(newRow, "ITEMSAPCODE", disp.itemCode, 1);
			Grids.mfgProcessDocDispEdit.SetValue(newRow, "MATNAME", disp.matName, 1);
			Grids.mfgProcessDocDispEdit.SetValue(newRow, "EXCRATE", disp.excRate, 1);
			Grids.mfgProcessDocDispEdit.SetValue(newRow, "INCRATE", disp.incRate, 1);
...
<div class="wrap_in" id="fixNextTag">
	...
	<section class="type01">
		...
		<div class="group01 mt20">
			...
			<c:if test="${calcType == '20'}"><c:set var="calcTypeText" value="기준수량 기준제품"/></c:if>
			<c:if test="${calcType == '30'}"><c:set var="calcTypeText" value="크림제품"/></c:if>
			<!-- 기준정보 start --------------------------------------------------------------------------------------------------------------------------------------------------------->
				...
				<table class="insert_proc01">
					...
					<tbody>
						...
						<tr>
							...
							<td colspan="3">
								<c:if test="${calcType == 10}">
									...
									<c:if test="${companyCode == 'MD'}">
										<input type="radio" name="calcType" id="radio_caclType1" value="10" checked="checked" onclick="changeCalcType(event)" disabled="disabled"/><label for="radio_caclType1"><span></span>프리믹스</label>
									</c:if>
								</c:if>
								...
								<!-- 
								<input type="hidden" id="currentCalcType" value="10">
								<input type="radio" name="calcType" id="radio_caclType1" value="10" checked="checked" onclick="changeCalcType(event)" disabled="disabled"/><label for="radio_caclType1"><span></span>일반제품</label>
								<input type="radio" name="calcType" id="radio_caclType2" value="20" onclick="changeCalcType(event)" disabled="disabled"><label for="radio_caclType2"><span></span>기준수량 기준제품</label>
								<input type="radio" name="calcType" id="radio_caclType3" value="30" onclick="changeCalcType(event)" disabled="disabled"><label for="radio_caclType3"><span></span>크림제품</label>
								 -->
							</td>
							<th>기준수량</th>
						...
						<tr>
							...
							<td>
								<c:if test="${calcType != '10'}">
									...
									( <input type="text" name="bagAmout" onkeyup="bagAmoutKeyUp(event)" style="width: 20%;" class="req" value="0" /> 포)
								</c:if>
								<c:if test="${calcType == '10'}">
									<input type="text" id="mixWeight" name="mixWeight" onkeyup="mixWeightKeyUp(event)" style="width: 30%;" class="req"/> kg
									( <input type="text" name="bagAmout" onkeyup="bagAmoutKeyUp(event)" style="width: 20%;" class="req" /> 포)
								</c:if>
								<%-- 
								<c:if test="${productType == 10}">
									<input type="text" id="mixWeight" name="mixWeight" style="width: 30%;" class="req" /> kg
									( <input type="text" name="bagAmout" style="width: 20%;" class="req" /> 포)
								</c:if>
								<c:if test="${productType != 10}">
									<input type="text" id="mixWeight" name="mixWeight" style="width: 30%;" value="0" class="read_only" readonly="readonly"/> kg
									( <input type="text" name="bagAmout" style="width: 20%;" value="0" class="read_only" readonly="readonly"/> 포)
								</c:if>
								 --%>
							</td>
							<th>BOM수율</th>
							<td><input type="text" id="productBomRatio" name="bomRate" onkeyup="productBomRateKeyup(event)" style="width: 30%;" class="req" value="0"/> %</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<th>품목제조보고서명</th>
							<td><input type="text" name="docProdName" style="width: 90%;" class="req" value="${productName}"/></td>
							<th>제조공정도 유형</th>
			...
			<div class="_tab03" style="position: relative;">
				...
				<div style="display: inline-block; width:96%; margin-left:2%">
					<ul id="subScroll" class="ul">
						...
						<%-- <a href="javascript:;" id="subProdAddBtn_1"><li class="select" onclick="changeTab(this)">
					...
					<table class="insert_proc01" style="border-top: none; border-right: 1px solid #d1d1d1">
						...
						<tbody>
							<tr>
								...
								<c:if test="${calcType == '10'}">
										<th>배합 총합</th><!-- 240724 -->
										<td><input type="text" name="subProdBomRateTotal" style="width: 100%;" readonly="readonly" class="read_only"/></td>
								</c:if>
								<th>기준수량</th>
								<td>
									<!-- <input type="text" name="subProdStdAmount" style="width: 100%;" readonly="readonly" class="read_only" onkeyup="setStdAmount(event)" /> -->
									<c:if test="${calcType == 20}">
										<input type="text" name="subProdStdAmount" style="width: 100%;" class="req" onkeyup="setStdAmount(event)"/>
									</c:if>
									<c:if test="${calcType != 20}">
				...
				<table class="insert_proc01">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<!-- <input type="text" id="qns" name="qns" style="width: 100%;" value="" class="req"/> -->
							</td>
			...
			<c:choose>
				<c:when test="${userUtil:getIsAdmin(pageContext.request) == 'Y' || userUtil:getUserGrade(pageContext.request) == '3'}">
					<c:set var="consumeDisplay" value="block"/>
				</c:when>
				<c:otherwise>
					<c:set var="consumeDisplay" value="none"/>
				</c:otherwise>
			</c:choose>
			<div id="consumeHeaderDiv" class="table_header07" style="display:${consumeDisplay}">
				<span class="table_order_btn">
					<button class="btn_up" onclick="moveUp(this)"></button><button class="btn_down" onclick="moveDown(this)"></button>
				</span>
				<span class="table_header_btn_box">
			...
			<table class="tbl05" style="display:${consumeDisplay}">
				<colgroup>
					<col width="20">
					<col width="140">
					<col />
				...
				<li id="dialog_create_li_design" class="mb5" style="display: block">
					...
					<dd style="width:77%">
						<div class="selectbox req" style="width:30%;">  
							...
							<label for="dialog_plantCode" id="label_dialog_plantCode">전체</label> 
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		subProdTabBtn = '<li id="subProdAddBtn_1" class="select" onclick="changeTab(this)" ondblclick="changeSubProdName(event)">'+$('#subProdAddBtn_1').html()+'</li>';
	...
	function calcBomAmountMix(element){
		...
		var productBomRatio = $('input[name=bomRate]').val();
		var calcType = $('input[name=calcType]:checked').val();
		...
		$('tbody[id^='+mixTbodyGroup+']').toArray().forEach(function(tbody){
		    ...
		    $('input[name=subProdDivWeight]').toArray().forEach(function(subWeight){
		        $(subWeight).keyup(); // call calcStdAmount(event);
		    })
		});
	...
	function saveValid(){
		...
		if($('input[name=adminWeightTo]').val().length == 0){
			alert('관리중량을 입력하세요');
			...
			success:function(data){
				if(data){
					...
					designDocDetail.sub.forEach(function(sub, subIndex){
						//if(subIndex > 0)
							//$('#subScroll').children('li:last').click()
						...
						sub.subMix.forEach(function(mix, mixIndex){
							...
							mix.subMixItem.forEach(function(item, itemIndex){
								...
								if( item.matStatus == 'B' ) {                           
		                           mixBody.children().css('background-color','#E0E0E0');
		                           //E0E0E0 회색
		                           //CCE5FF 파란색
		                           //FFDBDB 붉은색
		                           //FFFFCC 노란색
		                           //mixBody.children().addClass('m_visible1');                           
		                        } else if( item.matStatus == 'X' ) {
		                           mixBody.children().css('background-color','#FFDBDB');
		                           //mixBody.children().addClass('m_visible1');
		                        }
						...
						sub.subContent.forEach(function(cont, contIndex){
							...
							cont.subContentItem.forEach(function(item, itemIndex){
								...
								if( item.matStatus == 'B' ) {                           
		                           contBody.children().css('background-color','#E0E0E0');
		                           //E0E0E0 회색
		                           //CCE5FF 파란색
		                           //FFDBDB 붉은색
		                           //FFFFCC 노란색
		                           ...
		                           contBody.children().css('background-color','#FFDBDB');
		                           //mixBody.children().addClass('m_visible1');
		                        }
					...
					designDocDetail.pkg.forEach(function(pkg, pkgIndex){
						...
						if( pkg.matStatus == 'B' ) {                           
		                     pkgBody.children().css('background-color','#E0E0E0');
		                     //E0E0E0 회색
		                     //CCE5FF 파란색
		                     //FFDBDB 붉은색
		                     //FFFFCC 노란색
		                  } else if( pkg.matStatus == 'X' ) {
		                     pkgBody.children().css('background-color','#FFDBDB');
		                  }
		...
		$.ajax({
			...
			success:function(data){
				if(data){
					...
					//$('input[name=adminWeightFromUnit]').val(mgfProcDoc.adminWeightUnitFrom);
					//$('input[name=adminWeightToUnit]').val(mgfProcDoc.adminWeightUnitTo);
					
					$('#select_adminWeightUnitFrom option[value='+mgfProcDoc.adminWeightUnitFrom+']').prop('selected', true);
					$('label[for=select_adminWeightUnitFrom]').text(mgfProcDoc.adminWeightUnitFrom);
					$('#select_adminWeightUnitTo option[value='+mgfProcDoc.adminWeightUnitTo+']').prop('selected', true);
					$('label[for=select_adminWeightUnitTo]').text(mgfProcDoc.adminWeightUnitTo);
					...
					mgfProcDoc.sub.forEach(function(sub, subIndex){
						...
						sub.mix.forEach(function(mix, mixIndex){
							//console.log($('div[name=addMixDiv]'));
							$('div[name=addMixDiv]:last').click();
							...
							mix.item.forEach(function(item, itemIndex){
								...
								if( item.matStatus == 'B' ) {                           
		                           //mixBody.children().css('background-color','#E0E0E0');
									mixBody.children('tr:last').css('background-color','#E0E0E0');
		                           //E0E0E0 회색
		                           //CCE5FF 파란색
		                           //FFDBDB 붉은색
		                           //FFFFCC 노란색
		                           ...
		                           //mixBody.children().css('background-color','#FFDBDB');
									mixBody.children('tr:last').css('background-color','#FFDBDB');
		                           //mixBody.children().addClass('m_visible1');
		                        }
						...
						sub.cont.forEach(function(cont, contIndex){
							...
							cont.item.forEach(function(item, itemIndex){
								contHeader.children('span').children('button.btn_add_tr').click();
								if( item.matStatus == 'B' ) {                           
		                           contBody.children('tr:last').css('background-color','#E0E0E0');
		                           //E0E0E0 회색
		                           //CCE5FF 파란색
		                           //FFDBDB 붉은색
		                           //FFFFCC 노란색
		                           ...
		                           contBody.children('tr:last').css('background-color','#FFDBDB');
		                           //mixBody.children().addClass('m_visible1');
		                        }
					...
					// 표시사항 배햅비
					var isAutoDisp = $('input[name=isAutoDisp]').val();
					GridFunctions.Data.disp = mgfProcDoc.disp;
					...
					mgfProcDoc.disp.forEach(function(disp, dispIndex){
						...
						if(isAutoDisp == '1'){
							Grids.mfgProcessDocDispEdit.SetValue(newRow, "EXCRATEAUTO", disp.excRate, 1);
							Grids.mfgProcessDocDispEdit.SetValue(newRow, "INCRATEAUTO", disp.incRate, 1);
						}

					});
					setDispAttr(isAutoDisp);
					// 재료
					var pkgHeaderDiv = $('#packageHeaderDiv');
					var pkgBody = $('tbody[name=packageTbody]')
					pkgBody.empty();
					mgfProcDoc.pkg.forEach(function(pkg, pkgIndex){
						addRow(pkgHeaderDiv.children('span').children('button.btn_add_tr')[0], 'pkg');
						if( pkg.matStatus == 'B' ) {                           
                           pkgBody.children('tr:last').css('background-color','#E0E0E0');
                           //E0E0E0 회색
                           //CCE5FF 파란색
                           //FFDBDB 붉은색
                           //FFFFCC 노란색
                           ...
                           pkgBody.children('tr:last').css('background-color','#FFDBDB');
                           //mixBody.children().addClass('m_visible1');
                        }
	...
	function changeDispWeight(){
		...
		var dispWeightUnit = $('#select_dispWeightUnit').val();
			...
			success: function(data){
				if(data){
					...
					mfgDocList.forEach(function(obj, index){
						$('#select_mfgDoc').addOption(obj.dNo, '['+obj.plantName+'-'+obj.lineName+'] '+ restoreStr(restoreStrAmp(obj.memo)), false);
					});
				} else {
					return alert('제조공정서 조회 실패[1] - 시스템 담당자에게 문의하세요')
				}
